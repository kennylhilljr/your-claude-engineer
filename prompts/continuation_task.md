...
1. Use TransitionIssueState to move the issue to 'In Progress'
2. Add comment: 'Work started on this issue.'
Return: confirmation that issue is now In Progress."

**This MUST happen BEFORE Step 4c. The Linear board must reflect that work has begun.**
...
### Step 6: Move to Review (linear + Slack)

**Step 6a:** Delegate to the `linear` agent:
...
2. Use TransitionIssueState to move the issue to 'In Review'
...
2. Delegate to the `linear` agent:
   "Transition issue [key] to Done. Steps:
   1. Add detailed completion comment BEFORE transitioning:
   ...
   2. Use TransitionIssueState to move the issue to 'Done'
...
1. Delegate to the linear agent: "Add 'PROJECT COMPLETE' comment to META issue"
...
1. Delegate to the linear agent: "Add session summary comment to META issue with: what was completed, current progress counts, notes for next session"
